...
rm(by_ecoreg_so4, by_site_so4, by_year_so4)
...
input <- munged %>%
  ...
  mutate(hco3_meq = round(alk/100, digits = 4)) %>%
  ...
  select(sort, latitude, longitude, samp_site, subws, ecoregion, 
         ...
         op_meq, tp_meq, hco3_meq, ca_mg_rat, cation) 



# create an average ratio of ca_mg by ecoregion
...
# lm.ca <- lm(ca_meq ~ mg_meq + ecoregion, data = input)
# lm.ca <- augment(lm.ca) %>%
#  select(.rownames, .fitted) %>%
#  rename(ca_fit = .fitted) %>%
#  rename(sort = .rownames)
...
ggplot(lm.hard, aes(.fitted, ca_meq)) +
    geom_point() +
  geom_smooth(method = "lm") +
  theme_tufte() +
  ggtitle("Exploratory boxplot of ca:mg ratio")
...
hard.lm <- tidy(hard.lm) 


# create a tp-op relationship
phos.lm <- lm(op ~ tp, data = clean)
summary(phos.lm)
phos.lm <- augment(phos.lm)
