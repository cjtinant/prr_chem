      ...
      if_else(ecoregion == "BadLand", "Badl_E",
        if_else(.fittedPC3 < -1.25, "Tabl_LoF",
          if_else(.fittedPC1 < -0.5, "Tabl_LoHCO3",
             "Other")))))) %>%
  mutate(group_fin = str_c(group, ecoregion, sep = "_")) %>%
  mutate(group_fin = 
    if_else(group_fin == "Badl_E_BadLand", "Badl_E",
      if_else(group_fin == "Badl_W_BadLand", "Badl_W",
        if_else(group_fin == "CheyRiv_CheyRiv", "CheyRiv", 
            if_else(group_fin == "Other_TabLand", "Tbl_Othr", 
              if_else(group_fin == "Tabl_LoHCO3_TabLand", "Tbl_loHCO3", 
                if_else(group_fin == "Tabl_LoF_TabLand", "Tbl_loF", 
                   "SandH"))))))) %>%
  mutate(group_fin = as.factor(group_fin)) 
...
p4 <- ggplot(pca1.interp.sub, aes(.fittedPC1, .fittedPC3)) +
  geom_point(aes(shape = group_fin, color = group_fin)) +
  theme_tufte() +
  scale_y_reverse() +
  ylab("Inc. Ca ions ->") +
  xlab("Inc F ions ->")

#p0 <- autoplot(input.pca, data = pca1.interp, shape = "ecoregion",  
