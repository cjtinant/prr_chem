...
rm(by_ecoreg_hard, by_site_hard, by_year_hard, whr5_hard, by_month_hard)
# ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
...
rm(by_ecoreg_so4, by_month_so4, by_site_so4, by_year_so4)
...
rm(by_ecoreg_nate, by_month_nate, by_site_nate, by_year_nate)
...
rm(by_ecoreg_op, by_month_op, by_site_op, by_year_op)
```
...
input = left_join(clean, by_ecoreg_k) 
# rm(by_ecoreg_k)
...
input <- input %>%
  ...
  mutate(ca_meq = round(2*ca/40, digits = 2)) %>%
  mutate(mg_meq = round(2*mg/24.3, digits = 2)) %>%
  mutate(na_meq = round(na/23, digits = 2)) %>%
  mutate(k_meq = round(k/39, digits = 2)) %>%
  mutate(cl_meq = round(cl/35.5, digits = 2)) %>%
  mutate(f_meq = round(f/19, digits = 2)) %>%
  mutate(so4_meq = round(so4/96, digits = 2)) %>%
  mutate(nite_meq = round(nite/46, digits = 2)) %>%
  mutate(nate_meq = round(nate/62, digits = 2)) %>%
  mutate(op_meq = round(op/96, digits = 2)) %>%
  mutate(so4_meq = round(so4/96, digits = 2)) %>%
  mutate(tp_meq = round(tp/96, digits = 2)) %>%
  mutate(hco3_meq = round(alk/100, digits = 2)) %>%
  mutate(k_meq = if_else(is.na(k_meq), k_avg, k_meq)) %>%
  
  select(sort, latitude, longitude, samp_site, subws, ecoregion, 
         year, month, temp_c, ph, cond, hardness, ca_meq, mg_meq,
         na_meq, k_meq, cl_meq, f_meq, so4_meq, nite_meq, nate_meq,
         op_meq, tp_meq, hco3_meq) %>%
  mutate(cations = ca_meq + mg_meq + k_meq + na_meq) 

```


```{r}
ggplot(clean, aes(ca_meq, mg_meq)) +
  facet_wrap(~ecoregion) +
  geom_point() +
  geom_smooth(method = lm) +
  theme_tufte() +
  ggtitle("Exploratory boxplot of ca:mg ratio")

cation.lwr <- clean %>%
  filter(ecoregion == "LWR") %>%
  select(sort, mg_meq, ca_meq, na_meq)
  



# the ca: mg ratio looks pretty clear across the sites
hard.lm <- lm(mg ~ ca -1, data = hard.lwr)
hard.est <- 
```
