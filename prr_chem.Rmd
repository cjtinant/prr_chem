...
```{r box.cox.noCheyRiv}
# Remove the Cheyenne River stations
input.tert <- clean2 %>%
  filter(subws != "ChR") %>%
  select(-year) %>%
  select(-month) %>%
...
#~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
# calcium
ca <- boxcoxnc(input.tert$ca_meq, method = "sw", lam = seq(-3,3,0.01), 
                plotit = FALSE, alpha = 0.05, verbose = TRUE) 
ca.df <- as.data.frame(ca$lambda.hat) 
#~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
# magnesium
mg <- boxcoxnc(input.tert$mg_meq, method = "sw", lam = seq(-3,3,0.01), 
                plotit = FALSE, alpha = 0.05, verbose = TRUE) 
mg.df <- as.data.frame(mg$lambda.hat) 
#~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
# sodium
na <- boxcoxnc(input.tert$na_meq, method = "sw", lam = seq(-3,3,0.01), 
                plotit = FALSE, alpha = 0.05, verbose = FALSE) 
na.df <- as.data.frame(na$lambda.hat) 
#~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
# potassium
k <- boxcoxnc(input.tert$k_meq, method = "sw", lam = seq(-3,3,0.01), 
                plotit = FALSE, alpha = 0.05, verbose = FALSE) 
k.df <- as.data.frame(k$lambda.hat) 
#~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
# chloride
cl <- boxcoxnc(input.tert$cl_meq, method = "sw", lam = seq(-3,3,0.01), 
               plotit = FALSE, alpha = 0.05, verbose = FALSE) 
cl.df <- as.data.frame(cl$lambda.hat) 
#~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~ 
# fluoride
f <- boxcoxnc(input.tert$f_meq, method = "sw", lam = seq(-3,3,0.01), 
                 plotit = FALSE, alpha = 0.05, verbose = FALSE) 
f.df <- as.data.frame(f$lambda.hat) 
#~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~ 
# sulfate
so4 <- boxcoxnc(input.tert$so4_meq, method = "sw", lam = seq(-3,3,0.01), 
                 plotit = FALSE, alpha = 0.05, verbose = TRUE) 
so4.df <- as.data.frame(so4$lambda.hat) 
#~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~ 
# ratio of calcium to magnesium
ca_mg_rat <- boxcoxnc(input.tert$ca_mg_rat, method = "sw", lam = seq(-3,3,0.01), 
                  plotit = FALSE, alpha = 0.05, verbose = TRUE) 
ca_mg_rat.df <- as.data.frame(ca_mg_rat$lambda.hat) 
#
#~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
...
# transform the input.tert data to approach normal
input.tert.tr <-input.tert %>%
  rownames_to_column("newcol") %>%
  mutate(ca_meq = (ca_meq^ca$lambda.hat-1)/ca$lambda.hat) %>%
  mutate(mg_meq = (mg_meq^mg$lambda.hat-1)/mg$lambda.hat) %>%
  mutate(na_meq = (na_meq^na$lambda.hat-1)/na$lambda.hat) %>%
  mutate(k_meq = (k_meq^k$lambda.hat-1)/k$lambda.hat) %>%
  mutate(cl_meq = (cl_meq^cl$lambda.hat-1)/cl$lambda.hat) %>%
  mutate(f_meq = (f_meq^f$lambda.hat-1)/f$lambda.hat) %>%
  mutate(so4_meq = (so4_meq^so4$lambda.hat-1)/so4$lambda.hat) %>%
  mutate(ca_mg_rat = (ca_mg_rat^ca_mg_rat$lambda.hat-1)/ca_mg_rat$lambda.hat) 
         
rm(ca, ca_mg_rat, cl, f, k, mg, na, so4)
rm(ca.df, ca_mg_rat.df, cl.df, f.df, k.df, mg.df, na.df, so4.df)

# The scale() function centers and scales the data
input.tert.sc <- input.tert.tr %>%
  column_to_rownames(var = "newcol")
input.tert.sc <- as.data.frame(scale(input.tert.sc, center = TRUE, scale = TRUE))
```


```{r pairs.plot.noChR}
ggpairs(input.sc, title = "Transformed Major Anions and Cations - No ChRiv") +
  theme_tufte()
```
...
#mardiaTest(input.tr, qqplot = TRUE)
```
...
### PCA Results: Cheyenne River Stations Removed
```{r pca.tert}
# conduct a pca using the data with Cheyenne River removed
input.tert.pca <- prcomp(input.tert.sc, scale = TRUE, center = TRUE) 
summary(input.tert.pca)
```
...
# ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
cormat.tert <- cor(input.tert.sc)
eigdecomp.tert <- as.data.frame(eigen(cormat.tert))
eigdecomp.tert <- rownames_to_column(eigdecomp.tert)

eigdecomp.tert <- eigdecomp.tert %>%
  mutate(rowname = as.integer(rowname))
...
s2 <- ggplot(eigdecomp.tert, aes(rowname,values)) +
  geom_line() +
  geom_point() +
  geom_hline(yintercept = 1, lty=2) +
  theme_tufte() +
...
The first screeplot indicates the first three principal components are relevant and explain about 70% of the variance.  The second scree plot indicates the first four principal components are relevant and explain about 80% of the variance of the data.

```{r tidy.pca}
pca.all.eig <- tidy(input.pca, "variables")
pca.all.eig <- spread(pca.all.eig, key = PC, value = value) %>%
  select(1:4)
pca.all.au <- augment(input.pca, data = input)

pca.all.eig
```

PCA 1 describes a 
the ca-mg ratio alone, which is the same as the chey riv difference.  No new information from PCA1

```{r tidy.pca.tert}
pca.tert.eig <- tidy(input.tert.pca, "variables")
pca.tert.eig <- spread(pca.tert.eig, key = PC, value = value) %>%
  select(1:4)
pca.tert.au <- augment(input.tert.pca, data = input.tert)

pca.tert.eig
```

The PCA1 describes the calcium carbonate gradient.

```{r biplot}

p1 <- autoplot(input.pca, data = pca1.interp, shape = "group1",  
                 loadings = TRUE, loadings.label = TRUE) +
  theme_tufte() +
  ggtitle("Ordinate Plot Anions and Cations - All Data") +
  theme(legend.position = "none")

p2 <- autoplot(input.tert.pca, data = input.tert, shape = "class",  
                 loadings = TRUE, loadings.label = TRUE) +
  theme_tufte() +
...
factor2 <- clean2 %>%
  ...
  filter(samp_site != "CHR1") %>%
  filter(samp_site != "CHR2") %>%
...
pca2.interp <- left_join(pca2.interp, factor2, by="samp_site")
rm(factor1,factor2)
...
pca1.interp <- pca1.interp %>%
  ...
  mutate(group = if_else(.fittedPC1 < -4, "CheyRiv",
                         if_else(.fittedPC2 > 1.8, "group2", "group3")))
...
#factor2 <- data_frame(samp_site=c("AMH1", "BEA1", "BEA2", "BEA3", "BEL1",
#                                  "BEL2", "BLP1", "BLP2", "BUZ1", "COR1",
#                                  "CRA1", "EAN1", "EAN2", "LOD1", "LON1",
#                                  "LWR1", "LWR2", "LWR3", "LWR4", "MER1",
#                                  "MER3", "NFL1", "PAS1", "PAS2", "PAS3", 
#                                  "POR1", "POR2"POR3 POT1 RED1 WCC1 WCC2 
# WCC3 WHR1 WHR2 WHR3 WHR4 WOK1 WOK2 WOK3 WOK4 
...
```{r}
s1 <- ggplot(pca1.interp, aes(.fittedPC1, .fittedPC2)) +
  geom_point(aes(shape = group1)) +
  theme_tufte()
...
```{r}
s1 <- ggplot(pca.interp, aes(ecoregion, .fittedPC1)) +
  facet_wrap(~subws) +
  geom_boxplot() +
  theme_tufte()
