...
The results of the box-cox transformation indicates the data does not meet normality assumptions. 
...
The first screeplot indicates the first three principal components are relevant and explain about 77% of the variance.  
...
pca1.interp <- pca1.interp %>%
  ...
  mutate(group = if_else(.fittedPC1 < -4, "CheyRiv",
    if_else(.fittedPC2 > 1.8, "Badl_W",
      if_else(ecoregion == "BadLand", "Badl_E",
    ...
    if_else(final == "Badl_E_BadLand", "Badl_E",
      if_else(final == "Badl_W_BadLand", "Badl_W",
        if_else(final == "CheyRiv_CheyRiv", "CheyRiv", 
            if_else(final == "Other_TabLand", "Tbl_Othr", 
...
pca1.interp.sub <- pca1.interp %>%
  filter(ecoregion != "BadLand") %>%
  filter(ecoregion != "CheyRiv")
```

```{r plot.pca.axes}
autoplot(input.pca, data = pca1.interp, shape = "ecoregion",
...
```{r plot.results}
p1 <- ggplot(pca1.interp, aes(.fittedPC1, .fittedPC2)) +
  ...
  ggtitle("Interpretation of PCA Results for PRR Stream Water Chemistry",
          subtitle = "The lower right panel shows a detail of the group 'Other' shown in the other panels") +
  theme(legend.position = "none")
 
p2 <- ggplot(pca1.interp, aes(.fittedPC1, .fittedPC3)) +
  geom_point(aes(shape = group, color = group)) +
  theme_tufte() +
  xlab("Inc. HCO3 ions (54% of var.) ->") + 
  ylab("Inc F ions (10%) ->") +
...
p3 <- ggplot(pca1.interp, aes(.fittedPC3, .fittedPC2)) +
  geom_point(aes(shape = group, color = group)) +
  theme_tufte() +
  scale_y_reverse() +
  ylab("Inc. Ca ions (13%) ->") +
...
p4 <- ggplot(pca1.interp.sub, aes(.fittedPC1, .fittedPC3)) +
...
ggmap(map1) + 
  geom_point(aes(x = longitude, y = latitude, shape = ecoregion,
                 color = subws), data = map, size = 4) +
  theme_tufte() +
...
pca1.interp.map <- left_join(pca1.interp, locs)
  
pca1.interp.map <- pca1.interp.map %>%
  mutate(samp_site = as.factor(samp_site))



ggmap(map1) + 
  geom_point(aes(x = longitude, y = latitude, shape = final,
                 color = final), data = pca1.interp.map, size = 4) +
  theme_tufte() +
  ggtitle("Stream Sampling Stations for the Pine Ridge Reservation",
          subtitle = "Southwestern South Dakota") +
  theme(legend.position = "right")
...
pca.gp.map <- pca1.interp.map %>%
  group_by(samp_site) %>%
#  summarize(mean(ph))


p5 <- ggplot(map, aes(longitude, latitude)) +
  geom_point(size = 2.5, aes(shape = ecoregion, color = subws)) +
  theme_tufte() +
  scale_x_reverse() +
  ggtitle("Stream Sampling Stations for the Pine Ridge Reservation",
          subtitle = "Southwestern South Dakota") +
